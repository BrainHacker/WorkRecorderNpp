...
class CLocalization : public Singleton<CLocalization>
{
public:
...
protected:
    ...
    friend class Singleton<CLocalization>;
};
...
inline CString translate(uint controlId)
{
    CLocalization& localization = CLocalization::getInstance();
    return localization.LoadString(controlId);
}

inline void translateWindow(HWND hwnd, uint id)
{
    CLocalization& localization = CLocalization::getInstance();
    localization.Translate(hwnd, id);
}
